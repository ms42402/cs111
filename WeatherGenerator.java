...
public class WeatherGenerator {
    ...
    int month, double[][] drywet, double[][] wetwet) {
        ...
        int x = numberOfDaysInMonth[month - 2];
        ...
        for (row = 0; row < drywet.length; row++) {
            if (drywet[row][0] == longitute) {
                if (drywet[row][1] == latitude) {
                    pdry = drywet[row][month];
                }
            }
        }  
        for (row = 0; row < wetwet.length; row++) {
            if (wetwet[row][0] == longitute) {
                if (wetwet[row][1] == latitude) {
                    pwet = wetwet[row][month];
                }
            }
        }
        double rainNum = StdRandom.uniform(0.0, 1.0);
        if (rainNum <= 0.49 ) {
            forecast[0] = WET;
        }
        if ( ainNum > 0.49 ) {
        ...
        for (int i = 1; i < forecast.length; i++) {
            if(forecast[i-1] == WET) {
                wetSum = StdRandom.uniform(0.0,1.0); 
                if ( wetSum <= pwet) {
                    forecast[i] = WET;
                } else { 
                    forecast[i] = DRY;
                ...
                drySum = StdRandom.uniform(0.0,1.0);
                if (drySum <= pdry) {
                    forecast[i] = WET;
                } else {
                    forecast[i] = DRY;
    ...
    public static void main (String[] args) {
        ...
        StdRandom.setSeed(1604971462991L);
